...
from wzgram.types import CallbackQuery, ForceReply, InlineKeyboardButton, InlineKeyboardMarkup, Message
...
def _plan_info_text(mention: str, username_str: str, user_id: int, duration, price, currency="INR") -> str:
    ...
    return f"👤 <b>User:</b> {mention}\n🆔 <b>User ID:</b> <code>{user_id}</code>\n🔗 <b>Username:</b> {username_str}\n\n📦 <b>Plan:</b> {duration} days ({sym}{price})"
...
async def plan_selection(client: Client, callback_query: CallbackQuery):
    ...
    new_expiry = current_expiry + timedelta(days=int(duration)) if current_expiry and current_expiry > now else now + timedelta(days=int(duration))
    expiry_str = new_expiry.strftime("%Y-%m-%d %H:%M UTC")

    settings = SettingsManager.current()
    payment_instructions = settings.payment_instructions
    payment_qr_url = settings.payment_qr_url
    text = (f"<b>✅ Plan Selected: {plan['days']} Days</b>\n\n"
    ...
    text += "<b>After paying:</b> send your payment screenshot directly here (in this chat). The admin will review and activate your subscription."
    ...
    if dm_sent:
        ...
        await callback_query.message.reply_text(text + "\n\n⚠️ <i>Please start a DM with the bot first by clicking its username, then send your screenshot there.</i>", reply_markup=ForceReply(selective=True), quote=True)
        await callback_query.answer()
...
async def handle_payment_screenshot(client: Client, message: Message):
    if not SettingsManager.current().subscription:
        return
    sender_id = (message.from_user.id if message.from_user else None) or (message.sender_chat.id if message.sender_chat else None) or message.chat.id
    try:
        user = await db.get_user(sender_id)
        if not user or "pending_payment" not in user:
            await message.reply_text("ℹ️ We received your photo, but you don't have an active payment request.\n\nPlease use /start to select a subscription plan first, then send your payment screenshot.", quote=True)
            return
        pending = user["pending_payment"]
        duration, price, currency = pending.get("duration", "?"), pending.get("price", "?"), pending.get("currency", "INR")
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Approve", callback_data=f"approve_{sender_id}"), InlineKeyboardButton("❌ Reject", callback_data=f"reject_{sender_id}")]])
        ...
        admin_text = (f"<b>💰 New Payment Screenshot Received</b>\n\n<b>👤 User:</b> {user_mention}\n<b>🆔 User ID:</b> <code>{sender_id}</code>\n<b>🔗 Username:</b> {username_str}\n\n<b>📦 Plan Details:</b>\n  • Duration: <b>{duration} days</b>\n  • Price: <b>{_currency_symbol(currency)}{price}</b>\n\nPlease review the screenshot above and approve or reject.")
        admin_messages = []
        ...
        if admin_messages:
            await message.reply_text("✅ <b>Screenshot Received!</b>\n\nYour payment screenshot has been forwarded to the admin for review.\nYou will be notified once it's approved. Thank you! 🙏", quote=True)
        else:
            await message.reply_text("⚠️ Your screenshot was received but we could not reach the admin. Please contact the admin directly.", quote=True)
    except Exception as e:
        LOGGER.error(f"Error in handle_payment_screenshot: {e}")
        await message.reply_text(f"⚠️ Something went wrong while processing your screenshot. Please try again or contact the admin.\n\nError: {e}", quote=True)
...
async def admin_review(client: Client, callback_query: CallbackQuery):
    ...
    if action == "approve":
        ...
        try:
            ...
            addon_url = None
        try:
            invite_link = await client.create_chat_invite_link(chat_id=SettingsManager.current().subscription_group_id, member_limit=1, expire_date=datetime.utcnow() + timedelta(days=1))
            invite_text = f"\n\n🔗 <b>Group Invite:</b> {invite_link.invite_link}"
        except Exception:
            invite_text = ""
        expiry_str = user_data["subscription_expiry"].strftime("%Y-%m-%d")
        success_text = f"🎉 <b>Payment Approved!</b>\n\nYour subscription is now active until <b>{expiry_str}</b>.{invite_text}"
        if addon_url:
...
async def check_status(client: Client, message: Message):
    ...
    await message.reply_text(f"<b>Subscription Status:</b> Active ✅\n<b>Expiry Date:</b> {expiry.strftime('%Y-%m-%d %H:%M UTC')}\n<b>Time Remaining:</b> {days} days and {hours} hours")
