...
import httpx
from wzgram import Client, enums, filters
...
async def scrape_command(client: Client, message: Message) -> None:
    ...
    try:
        ...
        for i in range(4096, len(final_text), 4096):
            await message.reply_text(final_text[i:i + 4096], parse_mode=enums.ParseMode.HTML, disable_web_page_preview=True, quote=True)
    except Exception as error:
        await status.edit_text(f"<b>Failed to fetch data</b> \n<code>{error}</code>", parse_mode=enums.ParseMode.HTML)
