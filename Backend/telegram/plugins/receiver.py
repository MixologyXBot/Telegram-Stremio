...
def _base_from_doc(doc: dict) -> dict:
    return {
        "tmdb_id": doc.get("tmdb_id"), "imdb_id": doc.get("imdb_id"), "title": doc.get("title") or "",
        "year": doc.get("release_year") or 0, "rate": doc.get("rating") or 0, "description": doc.get("description") or "",
        "poster": doc.get("poster") or "", "backdrop": doc.get("backdrop") or "", "logo": doc.get("logo") or "",
        "genres": doc.get("genres") or [], "cast": doc.get("cast") or [], "runtime": str(doc.get("runtime") or ""),
        "original_language": doc.get("original_language"), "origin_country": doc.get("origin_country") or [],
    }
...
async def _handle_personal_session(client: Client, message: Message) -> None:
    ...
    async with manual_session_lock:
        ...
        metadata_info.update({"media_type": media_type, "quality": quality, "encoded_string": encoded,
                              "group_key": f"{channel}:{quality}:{split_key}" if split_key else None,
                              "part_number": resolved.get("part_number"), "is_anime": bool(doc.get("is_anime"))})
        ...
        if media_type == "tv":
            ...
            thumb_url = f"/thumb/{encoded}" if resolved.get("has_thumb") else ""
            metadata_info.update({"season_number": season_number, "episode_number": episode_number,
                                  "episode_title": f"S{season_number:02d}E{episode_number:02d}",
                                  "episode_backdrop": thumb_url or metadata_info.get("backdrop") or "",
                                  "episode_overview": "", "episode_released": ""})

        async with db_lock:
            updated_id = await db.insert_media(metadata_info, channel=p_channel, msg_id=p_msg, size=resolved["size"],
                                               name=name, raw_size=int(resolved.get("raw_size") or 0))
        if updated_id:
            where = (f"S{metadata_info['season_number']:02d}E{metadata_info['episode_number']:02d} " if media_type == "tv" else "")
...
async def file_receive_handler(client: Client, message: Message):
    ...
    try:
        ...
        metadata_info = await metadata(clean_filename(title), int(channel), msg_id,
                                       override_id=override_id or extract_default_id(message.caption or ""), season_hint=season_hint)
        ...
        await message.reply_text(text=f"Got Floodwait of {str(e.value)}s", disable_web_page_preview=True,
