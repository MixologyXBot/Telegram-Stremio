...
from Backend.helper.button_build import ButtonMaker
...
@Client.on_message(filters.command(["imdb", "imdbsearch"]))
async def imdb_search(client: Client, message: Message):
    if " " in message.text:
        k = await message.reply("<i>Searching IMDB ...</i>")
        ...
        if result := search(r"tt(\d+)", title, IGNORECASE):
            ...
            if movie := await asyncio.to_thread(get_movie, movieid):
                buttons.data_button(
            ...
            movies = await asyncio.to_thread(get_poster, title, bulk=True)
            if not movies:
                return await k.edit(
...
async def imdb_callback(client: Client, query: CallbackQuery):
    ...
    if user_id != int(data[1]):
        ...
        imdb = await asyncio.to_thread(get_poster, query=data[3], id=True)
        buttons = ButtonMaker()
        if imdb["trailer"]:
        ...
        # if not template:
        template = Telegram.IMDB_TEMPLATE
        if imdb and template != "":
            cap = template.format(**imdb, **locals())
        else:
            # Fallback default template if not configured
            cap = (
                f"<b>Title:</b> <a href='{imdb['url']}'>{imdb['title']}</a>\n"
                f"<b>Rating:</b> {imdb['rating']}\n"
                f"<b>Genres:</b> {imdb['genres']}\n"
                f"<b>Year:</b> {imdb['year']}\n"
                f"<b>Runtime:</b> {imdb['runtime']}\n"
                f"<b>Plot:</b> {imdb['plot']}"
            )
        ...
        if imdb.get("poster"):
            try:
                ...
                await query.message.reply_to_message.reply_photo(poster, caption=cap, reply_markup=buttons)
            except Exception as e:
                # If reply_to_message is somehow missing or other error, fallback
                await query.message.reply_photo(imdb["poster"], caption=cap, reply_markup=buttons)
